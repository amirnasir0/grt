...
import livelihoodImg from "../../assets/livelihood.png";

const currencySymbols = { USD: "$" };
const predefinedAmounts = [
  { value: 35, label: "Food Pack" },
  { value: 100, label: "Water Well" },
  { value: 150, label: "Medical suppies" },
  { value: 250, label: "Orphan Support" },
  { value: 400, label: "Livelihood Startup" },
  { value: 600, label: "Flood Relief Kit" },
];
...
function DonationCardInternal() {
  ...
  return (
    <div className="bg-white border border-[#A0A0A0] p-4 md:p-6 rounded shadow-lg text-[#1E1E1E] w-full">
      <h2 className="text-xl md:text-2xl font-bold mb-3 md:mb-4 text-center">India Relief Fund</h2>
      ...
      <div className="flex border rounded overflow-hidden mb-3 md:mb-4">
        {["one-off", "monthly"].map((type) => (
          <button
            ...
            className={`flex-1 py-2 text-sm md:text-base font-semibold transition ${
              planType === type
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-3 mb-3 md:mb-4">
        {predefinedAmounts.map(({ value, label }) => (
          <button
            ...
            className={`p-2 rounded border text-center flex flex-col items-center justify-center ${
...
function India() {
  const infoCards = [
    "Your donations to the 'Most Needy' fund have a significant impact on vulnerable families across India.",
    "Through your generosity, we are able to deliver emergency food supplies, healthcare and education support to those most in need.",
    "Your kindness builds long-term solutions in areas like clean water, food, medical access and livelihood.",
    "Even the smallest donation can change a life. Your compassion brings hope to those in urgent need.",
  ];

  const projects = [
    { title: "Clean Water for Villages", desc: "Provide clean, safe drinking water to remote Indian villages.", img: waterImg },
    { title: "Orphan Education Support", desc: "Sponsor education and school kits for underprivileged orphans.", img: orphansImg },
    { title: "Ramadan Food Pack India", desc: "Distribute essential food packs during Ramadan to families in need.", img: foodImg },
    { title: "Medical Aid Fund", desc: "Supply essential medicines and healthcare aid to poverty-stricken areas.", img: medicalImg },
    { title: "Flood Relief Camps", desc: "Support victims of monsoon floods with shelter, food, and medical aid.", img: mostUrgentImg },
    { title: "Livelihood Empowerment", desc: "Help poor families start small businesses and earn a stable income.", img: livelihoodImg },
  ];
  ...
  return (
    <section className="relative text-[#1E1E1E] overflow-hidden">
      ...
      <div
        ...
        style={{ backgroundImage: `url(${kashmirImg})` }}
      >
      ...
      <div className="relative z-10 bg-gradient-to-b from-transparent to-[#FAFAFA]">
        ...
        <div className="px-4 md:px-6 py-8 md:py-14 bg-gradient-to-b from-white/15 via-[#6EDFF6] to-[#6EDFF6] text-center">
          <h2 className="text-2xl md:text-3xl font-bold mb-6 md:mb-8">Help Those in Need Across India</h2>
